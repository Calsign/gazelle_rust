...
impl<'ast> AstVisitor<'ast> {
    ...
    fn visit_a_macro(&mut self, mac: &'ast syn::Macro) {
    ...
    fn visit_macro_tokens(&mut self, tokens: TokenStream) {
        ...
        if starts_with_ident(&tokens) {
            ...
            if let Some(fragment) = remove_prefix_until_ident(tokens) {
                self.visit_macro_tokens(fragment)
            }
        }
    }
            ...
            let scope_copy = Scope {
                mods: scope
                    ...
                    .map(|id| match id {
                        ...
                        Ident::Owned(ident) => Ident::Owned(ident.clone()),
                    })
                    .collect(),
                imports: scope
                    .imports
...
/// that `Ident`.
fn remove_prefix_until_ident(ts: TokenStream) -> Option<TokenStream> {
    let mut iter = ts.into_iter();
    let mut saw_non_ident = false;

    // Consume tokens until we find the first Ident
    while let Some(tt) = iter.next() {
        if let TokenTree::Ident(_) = &tt {
            if saw_non_ident {
                // Found a fresh Ident: put it back and collect the rest
                let mut rest = vec![tt];
                rest.extend(iter);
                return Some(rest.into_iter().collect());
            } else {
                // We haven't yet made any progress here
                continue;
            }
        } else {
            saw_non_ident = true;
